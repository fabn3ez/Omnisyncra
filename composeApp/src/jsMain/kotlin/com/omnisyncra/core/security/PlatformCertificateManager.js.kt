...
actual class PlatformCertificateManager {
    ...
    actual suspend fun securelyStoreCertificate(certificate: DeviceCertificate): Boolean {
        return try {
            ...
            window.localStorage.setItem("omnisyncra_cert_${certificate.deviceId}", json)
            true
        } catch (e: Exception) {
            false
        }
    ...
    actual suspend fun securelyRetrieveCertificate(deviceId: Uuid): DeviceCertificate? {
        return try {
            val json = window.localStorage.getItem("omnisyncra_cert_$deviceId") ?: return null
    ...
    actual suspend fun securelyDeleteCertificate(deviceId: Uuid): Boolean {
        return try {
            window.localStorage.removeItem("omnisyncra_cert_$deviceId")
            true
        } catch (e: Exception) {
            false
        }
...
/**
 * Serializable version of DeviceCertificate for JavaScript storage
 */
@Serializable
